...
if( $_POST["_method"] == "put" ) {
	...
	$cdate = date("Y-m-d H:i:s");
        $pdo->prepare("UPDATE `cases` set `state`=?,`expose`=?,`reason_code`=?,`expose_msg`=? where `id`=?")->execute(array($_POST["state"],'核發',$_POST["case"]["reason_code"],NULL,intval($_POST["id"])));
	if( $_POST["state"] == "reviewing" ) {
		        $pdo->prepare("UPDATE `cases` SET `state`=?,`expose`=?,`reason_code`=?,`review_user_id`=?,`review_department`=? where `id`=?")->execute(array('reviewing','核發',$_POST["case"]["reason_code"],$row_user["id"],$row_user["department"],intval($_POST["id"])));
		$msg = '從案件處理 > 案件審核('.$row_user["name"].')';
...
echo '<script>document.location.href="cases.php?page='.$_SESSION["page"].'&state=ready&n";</script>';
	...
	exit;
        ...
        $cdate = date("Y-m-d H:i:s");
        $pdo->prepare("UPDATE `cases` set `state`=?,`expose`=?,`expose_msg`=?,`reason_code`=? where `id`=?")->execute(array($_POST["state"],'退件',$_POST["cases"]["expose_msg"],NULL,intval($_POST["id"])));
        if( $_POST["state"] == "reviewing" ) {
                        $pdo->prepare("UPDATE `cases` set `state`=?,`expose`=?,`expose_msg`=?,`expose_note`=?,`review_user_id`=?,`review_department`=? where `id`=?")->execute(array('reviewing','退件',$_POST["cases"]["expose_msg"],$_POST["cases"]["expose_note"],$row_user["id"],$row_user["department"],intval($_POST["id"])));
                $msg = '從案件處理 > 案件審核('.$row_user["name"].')';
                $pdo->prepare("INSERT INTO `case_history` (`id`,`cases_id`,`user_id`,`message`,`cdate`) VALUES ( NULL, ?, ?, ?, ? )")->execute(array(intval($_POST["id"]),$row_user["id"],$msg,$cdate));
...
<?php
if( $_GET["commit"] == "搜尋" ) { ?>
	$('.back2list').attr('href', "casesall.php?min=<?=$_GET["min"];?>&max=<?=$_GET["max"];?>&page=<?=$_SESSION["page"];?>&state=<?=$_GET["state"];?>&full_car_number=<?=$_GET["full_car_number"];?>&commit=<?=$_GET["commit"];?>")
<?php } else { ?>
	$('.back2list').attr('href', "cases.php?min=<?=$_GET["min"];?>&max=<?=$_GET["max"];?>&page=<?=$_SESSION["page"];?>&state=<?=$_GET["state"];?>&full_car_number=<?=$_GET["full_car_number"];?>")
<?php } ?>
