...
if( $_POST["_method"] == "put" ) {
	...
	if( $_POST["state"] == "reviewing" ) {
		...
		echo '<script>document.location.href="cases2.php?page='.$_SESSION["page"].'&state=ready&n";</script>';
		...
		echo '<script>document.location.href="cases2.php?page='.$_SESSION["page"].'&state=reviewing&n";</script>';	
	} else {
		echo '<script>document.location.href="cases2.php?n";</script>';
	}
	exit;
	}
	} else {
...
?>
  ...
  <div class="row">
    <div class="col-lg-12">
	  ...
	  <a href="cases2.php?DetectLocation=<?=$_GET["DetectLocation"];?>&page=<?=$_SESSION["page"];?>" class="btn btn-primary back2list">回到列表</a>
...
<?php if( $row["DetectLocation"] == "固定式測速" ) { ?>
  <tr>
    <td>違規事由</td>
    <td><?php
  ...
  <tr>
    ...
    <span data-bip-type="input" data-bip-attribute="comment" data-bip-placeholder="<?php
                if( $row["comment"] != "" )
					...
					echo '點選新增備註';?>" data-bip-object="case" data-bip-ok-button="儲存" data-bip-cancel-button="取消" data-bip-skip-blur="false" data-bip-url="cases2_edit.php?id=<?=$_GET["id"];?>" class="best_in_place" id="best_in_place_case_198896_comment"></span>
    </td>
  </tr>
...
if( $row["review_user_id"] == "" ) echo $row["review_user_id"]?$row["review_user_id"]:$row_user["name"]; else echo $row["review_user_id"];?> ( <?=$row_dep["department"].$row_dep["unit"];?> )
    </td>
  </tr>
...
var snapshot = function( customElementName, $customIcon, item ) {
  if(item.mediaKind == 'video'){
    ...
    $.post("/hsinchu/cases2_edit.php", {format: 'json', file_name: file_name + ".png", base64_attachment: d}, function(data){
...
items: [{"src":"/CarSystem/<?=str_replace('\\','/',$row["PhotoURL"]);?>","srct":"/CarSystem/<?=str_replace('\\','/',str_replace('result','small_result',$row["PhotoURL"]));?>","title":"","customData":{"id":675340,"selected":true,"origin_url":"/CarSystem/<?=str_replace('\\','/',$row["PhotoURL"]);?>"}}<?php if( $row["VideoURL"] != "" ) { ?>,{"src":"/CarSystem/<?=str_replace('avi','mp4',str_replace('\\','/',$row["VideoURL"]));?>","srct":"/hsinchu/assets/video-player-e07c798bcb9faae5aae7ff042828949e27ff115e3bc8091446cd3a4ce8f0db38.png","thumbnailOpenImage":false,"title":"video","customData":{"id":675341,"selected":false,"origin_url":"http://220.130.189.235/CarSystem/<?=str_replace('\\','/',$row["VideoURL"]);?>"}}<?php } ?>],
  ...
  <script type="text/javascript">
    ...
    $(function(){
	    $('.back2list').attr('href', "cases2.php?DetectLocation=<?=$_GET["DetectLocation"];?>&page=<?=$_SESSION["page"];?>")
