...
if( $_GET["act"] == "del" && $_GET["id"] != "" ) {
	$pdo->prepare("delete from `illegalities` where `id`=?")->execute(array($_GET["id"]));
...
<?php
$result = $pdo->query("select * from `illegalities`");
$allRecrods = $result->rowCount();
?>
<form class="index-form" action="illegalities.php" accept-charset="UTF-8" method="post"><input name="utf8" type="hidden" value="&#x2713;" /><input type="hidden" name="authenticity_token" value="K3Vdn+pD1LWok+ZP4ZVAz0c+BCAiAdLMr7U+yVQD2CFL+YuUshqCTUBY7XKKx+l2e37/KSooDVBN3D02Qt5ojQ==" />
  <div class="card mb-3">
    <div class="card-header">
      <b>違規事實 列表</b>
      (共 <?=($allRecrods);?> 筆記錄)

      <a class="btn btn-success btn-sm float-right" style="margin-left: 20px;" href="illegalities_add.php">新增違規事實</a>
...
<?php
$i=0;
$result = $pdo->query("select * from `illegalities` order by `id`");
...
<?=$row["name"];?>                      
                    ...
                    <td width="5%" id="username_<?=$row["id"];?>">
<div class="list">
     <div class="fun_title">
...
?>
		  ...
		  <a href="illegalities_edit.php?id=<?=$row["id"];?>"><i class="far fa-edit"></i>編輯</a>&nbsp;|&nbsp;<a data-confirm="確定刪除?" rel="nofollow" data-method="delete" href="illegalities.php?id=<?=$row["id"];?>&act=del"><i class="fas fa-trash-alt"></i>刪除</a>
