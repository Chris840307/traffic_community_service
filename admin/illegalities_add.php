...
$nav = "違規事實";
$n=0;
if( isset( $_POST["report_user"]["name"])) {
        $name_msg="is-invalid";
...
print_r( $_POST );
if( $_POST["report_user"]["name"] != "" && $_POST["commit"] == "送出" ) {
if( $_POST["unexpose_reason"]["active"] == "1" )
	        $active = "t";
else
	        $active = "f";
        $cdate=date("Y-m-d H:i:s");
$pdo->prepare("INSERT INTO `sys_log` (`id`, `acc`, `name`, `class`, `ip`, `content`) VALUES (NULL, ?, ?, ?, ?, ?)")->execute(array($row_user["acc"],$row_user["name"],'違規事實',$ip,'新增成功'));

$pdo->prepare("INSERT INTO `illegalities` (`name`,`active`,`created_at`) VALUES ( ?,?,? )")->execute(array($_POST["report_user"]["name"],$active,$cdate));
...
<?php
$result = $pdo->query("select * from `illegalities` where `id`='".$_GET["id"]."'");
$row = $result->fetch(PDO::FETCH_ASSOC);
?>

          <form class="simple_form edit_report_user" id="edit_report_user_71" autocomplete="off" action="illegalities_add.php" accept-charset="UTF-8" method="post"><input name="utf8" type="hidden" value="&#x2713;" /><input type="hidden" name="_method" value="patch" /><input type="hidden" name="authenticity_token" value="/wTPxqg+6zcxzuHxwZP1+3OPowhW0MjTdLHN5Y8POkC5ZI3HV2cY7jn5v4wc1Li4n1XutES3czZYxOTwdmOvbw==" />
...
<div class="card mb-3">
    ...
    <div class="card-header">
      ...
      新增違規事實
      <div class="float-right">    <a class="btn btn-primary btn-sm" href="illegalities.php">回到列表</a>
...
<span class="divider"></span>
  ...
  <label class="col-sm-2 col-form-label col-form-label-md">違規事實名稱</label>
  <div class="col-sm-5">
      <input class="form-control is-valid required string optional form-control form-control-sm" autocomplete="off" type="text" value="" required name="report_user[name]" id="report_user_name" />
  </div>
</div>
<div class="form-group row">
