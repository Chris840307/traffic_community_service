...
<?php } ?>
  <td class="case_sn"><a href="/hsinchu/cases_edit.php?id=<?php echo $row["id"];?>"><?php echo $row["sn"];?></a></td>
...
<?php echo $row["created_at"];?>
            ...
            <td id="reason_name_<?php echo $row["id"];?>" class="case_reason_name">
<?php
...
?>
            ...
            <td id="full_address_<?php echo $row["id"];?>" class="case_full_address">
<?php
...
<?php if( $page > 1 ) { ?>
      <li class="page-item">
  <a class="page-link" href="cases.php">&laquo;</a>
</li>

      <li class="page-item">
      <a rel="prev" class="page-link" href="cases.php?page=<?= $prev;?>">&lsaquo;</a>
</li>
...
for($i=$spage; $i<=$page-1; $i++ ): ?>
    ...
    <a rel="next" class="page-link" href="cases.php?page=<?= $i; ?>"><?= $i; ?></a>
  </li>
<?php endfor; ?>
<?php } ?>
...
<?php } ?>
      ...
      <li class="page-item">
      <a rel="next" class="page-link" href="cases.php?page=<?= $next; ?>">&rsaquo;</a>
</li>

      <li class="page-item">
      <a class="page-link" href="cases.php?page=<?= $totoalPages;?>">&raquo;</a>
</li>
<?php  } ?>
...
<div class="hover_content" style="display: none;">
  ...
  break;
	case 'DetectLocation':
	var input_html = '<select class="search-value-input custom-select" name="' + value_input.attr('name') + '" id="' + value_input.attr('id') + '"><option>新竹火車站</option><option>城隍廟</option><option>移動式測速</option><option>巨城</option><option>慈雲</option><option>固定式測速</option></select>'
  search_condition.val('eq').change();
  search_condition.find('option').not('[value="eq"]').attr('hidden', '')
  wrapper.html(input_html);
  wrapper.find('.search-value-input').select2();
  ...
  function transform_cases(event) {
	...
	$('.index-form').attr('action', "cases.php?act=complete&page=<?=$_GET["page"];?>").submit();
      } else if( event == 'ready' ) {
        $('input[name="event"]').val(event);
        $('.index-form').attr('action', "cases.php?act=ready&page=<?=$_GET["page"];?>").submit();
      } else {
        $('input[name="event"]').val(event);
        $('.index-form').attr('action', "cases.php?page=<?=$_GET["page"];?>").submit();
      }
