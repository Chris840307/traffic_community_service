...
<form class="simple_form new_case" id="new_case" autocomplete="off" enctype="multipart/form-data" action="cases_w.php" accept-charset="UTF-8" method="post"><input name="utf8" type="hidden" value="&#x2713;" /><input type="hidden" name="authenticity_token" value="QgwM52PZJoMynmgIG2w6Munl7XlkQjIiMHa+n26B0l6keqRgNTkiSbmAM7u7mDsyiQnehbigxjQiU/wpM/njPQ==" />
  <div class="card mb-3">
...
<div class="bgc-white p-20 bd">
  <div class="mT-30">
    ...
    <div class="form-group row">
      <label class="col-sm-2 col-form-label">檢舉人姓名</label>
      <div class="col-sm-10 ">
    ...
    <div class="form-group row">
      <label class="col-sm-2 col-form-label">聯絡地址</label>
      <div class="col-sm-10">
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-10">
        <input class="form-control string optional datetimepicker form-control datetimepicker-input" data-toggle="datetimepicker" data-target="#case_created_at" type="text" name="case[created_at]" id="case_created_at" />
      </div>
    </div>
...
<?php
$resultc = $pdo->query("SELECT * FROM `illegalities` order by id");
while($rowc = $resultc->fetch(PDO::FETCH_ASSOC)){
echo '<option value="'.$rowc["name"].'">'.$rowc["name"].'</option>';
}
?>
    ...
    <div class="form-group row illegality_details_filed hide">
      <label class="col-sm-2 col-form-label">違規事實說明</label>
      <div class="col-sm-10 ">
  ...
  <div class="row">
    ...
    <div class="col-2 clear-file-btn">
      <a href="javascript:void(0)" onclick="clear_file_input(4)" ><i class="fas fa-trash-alt"></i> <label for="clear_file_input(4)">清除</label></a>
        ...
        每案上傳總容量限制為 80MB，如您係以<span class="text-danger">影片檢舉交通違規，請您擷取檢舉之違規片段即可，以避免影片容量過大無法上傳。</span>
        </small>
      </div>
    </div>
...
function validate_form(){
  ...
  $.validator.addMethod('createdatCheck', function(value, element, params) {
        var buffa = $("#case_created_at").val();
        var buffb = $("#case_violated_at").val();
    return !(new Date(buffa).getTime() <= new Date(buffb).getTime());
  }, "檢舉日期不能小於違規日期");

  $.validator.addMethod('violatedatCheck', function(value, element, params) {
        var buffa = $("#case_created_at").val();
        var buffb = $("#case_violated_at").val();
    return !(new Date(buffa).getTime() <= new Date(buffb).getTime());
  }, "違規日期不能大於檢舉日期");
...
function get_created_at(){
  return $('#case_created_at_date').val() + ' ' +$('#case_created_at_hour').val() + ':' + $('#case_created_at_min').val()
}

function get_car_number(){
  return $('#case_car_type_code option:selected').text() + ' ' + $('#case_first_car_number').val() + '-' + $('#case_last_car_number').val()
}
