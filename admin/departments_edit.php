...
<?php
$result = $pdo->query("select * from `department` where `id`='".$_GET["id"]."'");
$row = $result->fetch(PDO::FETCH_ASSOC);
?>

          <form class="simple_form edit_report_user" id="edit_report_user_71" autocomplete="off" action="departments_update.php" accept-charset="UTF-8" method="post"><input name="utf8" type="hidden" value="&#x2713;" /><input type="hidden" name="_method" value="patch" /><input type="hidden" name="authenticity_token" value="/wTPxqg+6zcxzuHxwZP1+3OPowhW0MjTdLHN5Y8POkC5ZI3HV2cY7jn5v4wc1Li4n1XutES3czZYxOTwdmOvbw==" />
...
<div class="card mb-3">
    ...
    <div class="card-header">
      ...
      編輯單位
      <div class="float-right">    <a class="btn btn-primary btn-sm" href="departments.php">回到列表</a>
...
<span class="divider"></span>
  ...
  <label class="col-sm-2 col-form-label col-form-label-md">單位名稱</label>
  <div class="col-sm-5">
  <?=$row["dname"];?> / <?=$row["sname"];?>
  </div>
</div>
    <div class="form-group row">
  <label class="col-sm-2 col-form-label col-form-label-md">單位電話</label>
  <div class="col-sm-5">
      <input class="form-control is-valid required string optional form-control form-control-sm" autocomplete="off" type="text" value="<?=$row["dphone"];?>" name="report_user[title]" id="report_user_title" />
  </div>
</div>
