...
<form class="search-form" id="case_search" autocomplete="off" action="cases2.php?DetectLocation=<?=$_GET["DetectLocation"];?>" accept-charset="UTF-8" method="post">
<?php } else { ?>
<form class="search-form" id="case_search" autocomplete="off" action="cases2.php?DetectLocation=<?=$_GET["DetectLocation"];?>&state=<?=$_GET["state"];?>" accept-charset="UTF-8" method="post">
<?php } ?>
...
?>
	<td><?php
if( $mul != 0 ) {
echo '<a href="cases2.php?DetectLocation='.$_GET["DetectLocation"].'&full_car_number='.$row["full_car_number"].'&min='.$row["created_at"].'&max='.$day7.'">'.$mul.'</a>';
} else {
echo '';
}
?></td>
...
<?php } ?>
  <td class="case_sn"><a href="/hsinchu/cases2_edit.php?DetectLocation=<?=$_GET["DetectLocation"];?>&id=<?php echo $row["id"];?>"><?php echo $row["sn"];?></a></td>
...
$review_info = $pdo->query("select * from `user_list` where `name`='".$row["review_user_id"]."'");
$row_dep = $review_info->fetch(PDO::FETCH_ASSOC);
echo $row_dep["department"].$row_dep["unit"];
} else {
...
<?php if( $page > 1 ) { ?>
      <li class="page-item">
  <a class="page-link" href="cases2.php?DetectLocation=<?=$_GET["DetectLocation"];?>">&laquo;</a>
</li>

      <li class="page-item">
      <a rel="prev" class="page-link" href="cases2.php?DetectLocation=<?=$_GET["DetectLocation"];?>&page=<?= $prev;?>">&lsaquo;</a>
</li>
...
for($i=$spage; $i<=$page-1; $i++ ): ?>
    ...
    <a rel="next" class="page-link" href="cases2.php?DetectLocation=<?=$_GET["DetectLocation"];?>&page=<?= $i; ?>"><?= $i; ?></a>
  </li>
<?php endfor; ?>
<?php } ?>
...
<?php } ?>
      ...
      <li class="page-item">
      <a rel="next" class="page-link" href="cases2.php?DetectLocation=<?=$_GET["DetectLocation"];?>&page=<?= $next; ?>">&rsaquo;</a>
</li>

      <li class="page-item">
      <a class="page-link" href="cases2.php?DetectLocation=<?=$_GET["DetectLocation"];?>&page=<?= $totoalPages;?>">&raquo;</a>
</li>
<?php  } ?>
...
<div class="hover_content" style="display: none;">
  ...
  function review_case(id) {
    ...
    $.post("/hsinchu/admin/cases/batch_expose", data)
  }
  function transform_cases(event) {
    if ($('.select_checkbox:checked').length > 0) {
      if (event == 'review') {
        $.post('/hsinchu/admin/cases/batch_expose', $('.index-ftransform_casesorm').serialize())
        ...
        if ($('[name="cases[department_id]"]').val() == '') {
          ...
          $.post("/hsinchu/admin/cases/assign", { case_ids: ids, department_id: department_id })
        }
      } else if( event == 'complete' ) {
        $('input[name="event"]').val(event);
	$('.index-form').attr('action', "cases2.php?act=complete&page=<?=$_GET["page"];?>").submit();
      } else if( event == 'ready' ) {
        $('input[name="event"]').val(event);
        $('.index-form').attr('action', "cases2.php?act=ready&page=<?=$_GET["page"];?>").submit();
      } else {
        $('input[name="event"]').val(event);
        $('.index-form').attr('action', "cases2.php?page=<?=$_GET["page"];?>").submit();
      }
