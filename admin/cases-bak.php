...
//$result = $pdo->query("select * from `violation`");
while($row = $result->fetch(PDO::FETCH_ASSOC)){
	$carnumber = explode('-',$row["CarNumber"]);
	$sn_date = str_replace('-','',substr($row["Datetime"],0,10));
	$check_sn = $pdo->query('select * from `cases` where `sn` like "%'.$sn_date.'%" order by `sn` desc limit 1');
...
<?php
$i=0;
$result = $pdo->query("select * from `cases` $sqlx order by `created_at` desc LIMIT $paginationStart, $limit");
while($row = $result->fetch(PDO::FETCH_ASSOC)){
$i++;
?>
...
<?php echo $row["Location"];?>
	    ...
	    第二分局 > <?php echo $car_array["state"][$row["state"]];?> (<?=$row["review_user_id"];?> )
            </td>
