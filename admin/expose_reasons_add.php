...
$nav="舉發條款";
$n=0;
if( $_POST["report_user"]["code"] == "" && isset( $_POST["report_user"]["name"])) {
	$name_msg="is-invalid";
	$name_msg2='<div class="invalid-feedback">資料尚未填寫完整</div>';
	$n = 1;
}

if( isset( $_POST["report_user"]["name"]) && $n == 0 ) {
	$cdate=date("Y-m-d H:i:s");
$pdo->prepare("INSERT INTO `sys_log` (`id`, `acc`, `name`, `class`, `ip`, `content`) VALUES (NULL, ?, ?, ?, ?, ?)")->execute(array($row_user["acc"],$row_user["name"],'舉發條款',$ip,'新增成功'));
...
$pdo->prepare("INSERT INTO `expose_reasons` (`oid`,`code`,`name`,`active`,`created_at`) VALUES ( ?,?,?,?,? )")->execute(array('0',$_POST["report_user"]["code"],$_POST["report_user"]["name"],$active,$cdate));
...
<span class="divider"></span>
  ...
  <label class="col-sm-2 col-form-label col-form-label-md">法條代碼</label>
  <div class="col-sm-5">
  <input class="form-control <?=$username_msg;?> string required form-control form-control-sm" autocomplete="off" required="required" aria-required="true" type="text" name="report_user[code]" id="report_user_code" value="<?=$_POST["report_user"]["code"];?>"/> 舉例: 5510404
<?=$username_msg2;?>
  </div>
</div>
    <div class="form-group row">
  <label class="col-sm-2 col-form-label col-form-label-md">法條說明</label>
  <div class="col-sm-5">
  <input class="form-control <?=$username_msg;?> string required form-control form-control-sm" autocomplete="off" required="required" aria-required="true" type="text" name="report_user[name]" id="report_user_name" value="<?=$_POST["report_user"]["name"];?>"/>
<?=$username_msg2;?> 舉例: 併排臨時停車
  </div>
</div>
<div class="form-group row">
...
<div class="form-actions">
  ...
  <a class="btn btn-secondary" href="expose_reasons.php">取消</a>
</div>
