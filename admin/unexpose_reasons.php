...
<?php
$result = $pdo->query("select * from `unexpose_reasons`");
...
<form class="index-form" action="reason_list.php" accept-charset="UTF-8" method="post"><input name="utf8" type="hidden" value="&#x2713;" /><input type="hidden" name="authenticity_token" value="K3Vdn+pD1LWok+ZP4ZVAz0c+BCAiAdLMr7U+yVQD2CFL+YuUshqCTUBY7XKKx+l2e37/KSooDVBN3D02Qt5ojQ==" />
  <div class="card mb-3">
    <div class="card-header">
      ...
      <a class="btn btn-success btn-sm float-right" style="margin-left: 20px;" href="unexpose_reasons_add.php">新增退件原因</a>
...
<?php
$i=0;
$result = $pdo->query("select * from `unexpose_reasons` order by `updated_at` desc");
...
<?php
if( mb_strlen( $row["name"] ) >= 50 )
	echo mb_substr($row["name"],0,50,"utf-8").'....';
else
	echo $row["name"];
?>                      
                    </td>
                    <td width="4%" id="username_<?=$row["id"];?>">
<div class="list">
     <div class="fun_title">
            ...
            class="ad_on" title="點擊不顯示"<?php }else{?>class="ad_off" title="點擊顯示"<?php }?>></span>
                    ...
                    <td width="2%" id="created_at_<?=$row["id"];?>">
<?php
	echo $row["updated_at"];	
?>
                    </td>
		  ...
		  <a href="unexpose_reasons_edit.php?id=<?=$row["id"];?>"><i class="far fa-edit"></i>編輯</a>&nbsp;<!--|&nbsp;<a data-confirm="確定刪除?" rel="nofollow" data-method="delete" href="departments.php?id=<?=$row["id"];?>"><i class="fas fa-trash-alt"></i>刪除</a>-->
