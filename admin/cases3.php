...
//過濾假日 11-19 新竹市東區中央路、民生路口 - 未依標誌標線行駛 判斷
$result = $pdo->query("select * from `cases` where `cam_place`='新竹市東區中央路、民生路口' and `cam_reason`='未依標誌標線行駛' and `create_date` is NULL");
...
/*
$result = $pdo->query("select `id`,`pic1` from `cases` where `DetectLocation`='中華中正東'");
while($row = $result->fetch(PDO::FETCH_ASSOC)){
	$buff = '/var/www/html/CarSystem/';
	$VideoURL=str_replace('.jpg','.mp4',$row["pic1"]);
	$delf = $buff.$VideoURL;
	if( file_exists($delf)) {
		unlink( $delf );
	}
...
if( $_GET["DetectLocation"] == "竹東" )
	$nav = "flag6";
else if( $_GET["DetectLocation"] == "楊梅" )
	$nav = "flag7";
else if( $_GET["DetectLocation"] == "泰安" )
        $nav = "flag8";
else if( $_GET["DetectLocation"] == "民權建國" )
	        $nav = "flag9";
else if( $_GET["DetectLocation"] == "和平西園" )
                $nav = "flag10";
else if( $_GET["DetectLocation"] == "民權蘭州" )
	        $nav = "flag11";
else if( $_GET["DetectLocation"] == "市民林森2" )
                $nav = "flag12";
else if( $_GET["DetectLocation"] == "市民建國" )
                $nav = "flag13";
else if( $_GET["DetectLocation"] == "忠孝基隆" )
                $nav = "flag14";
else if( $_GET["DetectLocation"] == "信義建國" )
                $nav = "flag15";
else if( $_GET["DetectLocation"] == "民權承德" )
                $nav = "flag16";
else if( $_GET["DetectLocation"] == "市民建國2" )
	        $nav = "flag17";
else
	$nav = "flagx";
$result = $pdo->query("select * from `violation` where `DetectLocation`='新竹火車站' and `flag`='0' order by `RowNo` asc");
while($row = $result->fetch(PDO::FETCH_ASSOC)){
...
if( $_POST["act"] == "cam_ip" ) {
	if( $_POST["cam_ip"] != "" ) {
		$cam_id = time();
        $_SESSION['formdata'][2]['a'][0]['name']='cam_ip';
	$_SESSION['formdata'][2]['p']='eq';
	$_SESSION['formdata'][2]['v'][0]['value']=$_POST["cam_ip"];
	} else {
...
<form class="search-form" id="case_search" name="case_search" autocomplete="off" action="cases3.php?DetectLocation=<?=$_GET["DetectLocation"];?>" accept-charset="UTF-8" method="post">
<?php } else { ?>
<form class="search-form" id="case_search" name="case_search" autocomplete="off" action="cases3.php?DetectLocation=<?=$_GET["DetectLocation"];?>&state=<?=$_GET["state"];?>" accept-charset="UTF-8" method="post">
<?php } ?>
...
if( $_GET["min"] != "" && $_GET["max"] != "" && $_GET["full_car_number"] != "" ) {
        $result = $pdo->query("select * from `cases` where `DetectLocation`='".$_GET["DetectLocation"]."' and `full_car_number`='".$_GET["full_car_number"]."' and `created_at`>='".$_GET["max"]."' and `created_at`<='".$_GET["min"]."'");
} else {
	$result = $pdo->query("select * from `cases` where `DetectLocation`='".$_GET["DetectLocation"]."' $sqlx");
}
...
<?php 
if( $row_user["flag8"] == "1" && $nav == "flag8" ) { ?>
<a class="btn btn-success btn-sm float-right" href="cases_new.php">新增案件</a>
<?php } ?>
  </div>
  <div class="card-body">
...
<th><a class="select_all" href="#">全選</a></th>
<?php } ?>
            <!--<th>
	                  重覆篩選
            </th>-->
...
<th>
            <form action="cases3.php?DetectLocation=<?=$_GET["DetectLocation"];?>" method="post">
<input type="hidden" name="act" value="cam_reason">
            <select class="search-value-input custom-select" name="cam_reason" onchange="this.form.submit();">
<option value="">違規事由(全部)</option>
<?php
$result_ip = $pdo->query("SELECT `cam_reason` FROM `cases` where `DetectLocation`='".$_GET["DetectLocation"]."' GROUP BY `cam_reason`");
while($row_ip = $result_ip->fetch(PDO::FETCH_ASSOC)){
        $resultn = $pdo->query("SELECT `cam_reason`,`Location` FROM `cases` where `cam_reason`='".$row_ip["cam_reason"]."' limit 1");
        $rown = $resultn->fetch(PDO::FETCH_ASSOC);
        if( $_SESSION['formdata'][3]['v'][0]['value'] == $row_ip["cam_reason"] )
                echo '<option value="'.$row_ip["cam_reason"].'" selected>'.$row_ip["cam_reason"].'</option>';
        else
                echo '<option value="'.$row_ip["cam_reason"].'">'.$row_ip["cam_reason"].'</option>';
}
?></select>
</form>
</th>
...
if( $row_user["flag6"] == "1" ) {
        if( $sqlz != "" )
                $sqlz .= " or ";
        $sqlz .= "`DetectLocation`='中華興隆'";
}
if( $row_user["flag7"] == "1" ) {
                if( $sqlz != "" )
                                        $sqlz .= " or ";
                        $sqlz .= "`DetectLocation`='文興自強南'";
}
if( $row_user["flag8"] == "1" ) {
                if( $sqlz != "" )
                                        $sqlz .= " or ";
                        $sqlz .= "`DetectLocation`='中華中正東'";
}
if( $row_user["flag9"] == "1" ) {
                if( $sqlz != "" )
                                        $sqlz .= " or ";
                        $sqlz .= "`DetectLocation`='民權建國'";
}
if( $row_user["flag10"] == "1" ) {
                if( $sqlz != "" )
                                        $sqlz .= " or ";
                        $sqlz .= "`DetectLocation`='和平西園'";
}
if( $row_user["flag11"] == "1" ) {
                if( $sqlz != "" )
                                        $sqlz .= " or ";
                        $sqlz .= "`DetectLocation`='民權蘭州'";
}
if( $row_user["flag12"] == "1" ) {
                if( $sqlz != "" )
                                        $sqlz .= " or ";
                        $sqlz .= "`DetectLocation`='市民林森2'";
}
if( $row_user["flag13"] == "1" ) {
                if( $sqlz != "" )
                                        $sqlz .= " or ";
                        $sqlz .= "`DetectLocation`='市民建國'";
}
if( $row_user["flag14"] == "1" ) {
                if( $sqlz != "" )
                                        $sqlz .= " or ";
                        $sqlz .= "`DetectLocation`='忠孝基隆'";
}
if( $row_user["flag15"] == "1" ) {
                if( $sqlz != "" )
                                        $sqlz .= " or ";
                        $sqlz .= "`DetectLocation`='信義建國'";
}
if( $row_user["flag16"] == "1" ) {
                if( $sqlz != "" )
                                        $sqlz .= " or ";
                        $sqlz .= "`DetectLocation`='民權承德'";
}
if( $row_user["flag17"] == "1" ) {
                if( $sqlz != "" )
                                        $sqlz .= " or ";
                        $sqlz .= "`DetectLocation`='市民建國2'";
}

?>
            <form action="cases.php?state=<?=$_GET["state"];?>" method="post">
<input type="hidden" name="act" value="create_date">
            <select class="search-value-input custom-select" name="create_date" onchange="this.form.submit();">
<option value="">違規日期／時間(全部)</option>
<?php

$result_ip = $pdo->query("SELECT `create_date` FROM `cases` where $sqlz GROUP BY `create_date` order by create_date desc");
while($row_ip = $result_ip->fetch(PDO::FETCH_ASSOC)){
        $resultn = $pdo->query("SELECT `create_date` FROM `cases` where `create_date`='".$row_ip["create_date"]."' limit 1");
        $rown = $resultn->fetch(PDO::FETCH_ASSOC);
        if( $_SESSION['formdata'][4]['v'][0]['value'] == $row_ip["create_date"] )
                echo '<option value="'.$row_ip["create_date"].'" selected>違規日期: '.$row_ip["create_date"].'</option>';
        else
                echo '<option value="'.$row_ip["create_date"].'">違規日期: '.$row_ip["create_date"].'</option>';
}
?></select></form>
            </th>
            <th>
            ...
            <th>
	    <form action="cases3.php?DetectLocation=<?=$_GET["DetectLocation"];?>" method="post">
<input type="hidden" name="act" value="cam_ip">
	    <select class="search-value-input custom-select" name="cam_ip" onchange="this.form.submit();">
<option value="">違規地點(全部)</option>
<?php
$result_ip = $pdo->query("SELECT `cam_ip` FROM `cases` where `DetectLocation`='".$_GET["DetectLocation"]."' GROUP BY `cam_ip` order by cam_ip asc");
while($row_ip = $result_ip->fetch(PDO::FETCH_ASSOC)){
        $resultn = $pdo->query("SELECT `cam_ip`,`Location` FROM `cases` where `cam_ip`='".$row_ip["cam_ip"]."' order by created_at desc limit 1");
        $rown = $resultn->fetch(PDO::FETCH_ASSOC);
	if( $rown["Location"]!= "" && $row_ip["cam_ip"] != "" ) {
	if( $_SESSION['formdata'][2]['v'][0]['value'] == $row_ip["cam_ip"] )
        	echo '<option value="'.$row_ip["cam_ip"].'" selected>'.$row_ip["cam_ip"].$rown["Location"].'</option>';
	else
		echo '<option value="'.$row_ip["cam_ip"].'">'.$row_ip["cam_ip"].$rown["Location"].'</option>';
	}
}
?></select>
</form>
...
if( $_GET["min"] != "" && $_GET["max"] != "" && $_GET["full_car_number"] != "" ) {
	$result = $pdo->query("select * from `cases` where `DetectLocation`='".$_GET["DetectLocation"]."' and `full_car_number`='".$_GET["full_car_number"]."' and `created_at`>='".$_GET["max"]."' and `created_at`<='".$_GET["min"]."' order by `created_at` desc LIMIT $paginationStart, $limit");
} else {
	$result = $pdo->query("select * from `cases` where `DetectLocation`='".$_GET["DetectLocation"]."' $sqlx order by `created_at` desc LIMIT $paginationStart, $limit");
}
//$result = $pdo->query("select * from `cases` where `DetectLocation`='".$_GET["DetectLocation"]."' $sqlx order by `created_at` desc LIMIT $paginationStart, $limit");
while($row = $result->fetch(PDO::FETCH_ASSOC)){
$i++;
...
?>
  ...
  <td><?php echo $i;?></td>
<?php
	$day7 = date('Y-m-d H:i:s',strtotime($row["created_at"] . "-7 days"));
$resultm = $pdo->query("select * from `cases` where `DetectLocation`='".$_GET["DetectLocation"]."' and `full_car_number`='".$row["full_car_number"]."' and `id`!='".$row["id"]."' and `created_at`>='".$day7."' and `created_at`<='".$row["created_at"]."'");
$mul=0;
...
if( $mul != 0 && $row["full_car_number"]!="") {
	echo '<a href="cases3.php?DetectLocation='.$_GET["DetectLocation"].'&full_car_number='.$row["full_car_number"].'&min='.$row["created_at"].'&max='.$day7.'">'.$mul.'</a>';
} else {
	echo '';
}
?></td>-->
<?php if( $_GET["state"] == "reviewing" || $_SESSION['formdata'][0]['v'][0]['value'] == "reviewing" ) { ?>
           <td
              onclick="(function(e, obj){ if(e.target.type !== 'checkbox') {$(':checkbox', obj).trigger('click'); if($(':checkbox', obj).is(':checked')){$(obj).addClass('active')}else{$(obj).removeClass('active')};} })(event, this)">
	      <input type="checkbox" name="case_ids[]" value="<?=$row["id"];?>" class="select_checkbox" />
            </td>
<?php } ?>
  <td class="case_sn"><a href="/wos/cases3_edit.php?DetectLocation=<?=$_GET["DetectLocation"];?>&id=<?php echo $row["id"];?>"><?php echo $row["sn"];?></a>
<?php
...
?>
            ...
            <td id="cam_reason_<?php echo $row["id"];?>" class="cam_reason" style="width:140px;">
<?php echo $row["cam_reason"];?>
            </td>
            <td id="car_type_code_<?php echo $row["id"];?>" class="case_full_car_number" style="width:80px;">
<?php echo $row["car_type_code"];?>
            </td>
            <td id="full_car_number_<?php echo $row["id"];?>" class="case_full_car_number">
<?php echo $row["full_car_number"];?>
            </td>
            <td id="created_at_<?php echo $row["id"];?>" class="case_created_at">
<?php echo $row["created_at"];?>
            </td>
		    <td id="reason_name_<?php echo $row["id"];?>" class="case_reason_name" style="text-overflow: ellipsis;" title="<?=$row["reason_code"];?>">
<?php
$msg = explode(' - ',$row["reason_code"]);
echo $msg[1];
?>
            ....</td>
            <td id="reason_code_<?php echo $row["id"];?>" class="case_reason_code">
<?php 
echo $msg[0];
?>
            </td>
            <td id="full_address_<?php echo $row["id"];?>" class="case_full_address" style="width:280px;">
<?php
if( $_GET["DetectLocation"] == "固定式測速" || $row["DetectLocation"] == "巨城" ) {
	echo $row["cam_place"];
} else {
if( $row["car_place"] == null || $row["car_place"] == "" )
        echo $row["DetectLocation"];
else
	echo $row["cam_ip"].$row["car_place"];

}?>
            </td>
            <td id="state_desc_<?php echo $row["id"];?>" class="case_state_desc">
	    <?php
if( $row["review_department"] == null ) {
if( $row["DetectLocation"] == "新竹都城隍廟周邊道路" ) {
	echo "第一分局";
} else if( $row["DetectLocation"] == "新竹火車站" ) {
	echo "第二分局";
} else if( $row["DetectLocation"] == "固定式測速" ) {
echo "交通警察隊";
}} else {
echo $row["review_department"];
	}
?>
 > <?php echo $car_array["state"][$row["state"]];?> (<?=$row["review_user_id"];?> )
            </td>
<?php
if( $_GET["DetectLocation"] == "移動式測速" ) {
?>
<td id="complete_date_<?php echo $row["id"];?>" class="case_deadline_date">
...
<?php echo date('Y-m-d',strtotime("$row[created_at] +14 day"));?>
            </td>
<?php } ?>
	  </tr>
<?php
...
<?php if( $_GET["state"] == "reviewing" || $_SESSION['formdata'][0]['v'][0]['value'] == "reviewing" ) { ?>
  ...
  <a onclick="transform_cases(&#39;complete&#39;)" class="btn btn-primary btn-sm" data-remote="true" href="#">案件結束</a>
  <a onclick="transform_cases(&#39;ready&#39;)" class="btn btn-danger btn-sm" data-remote="true" href="#">案件退回</a>
</div>
<?php } ?>
</div>
</form>
...
<?php if( $page > 1 ) { ?>
      <li class="page-item">
  <a class="page-link" href="cases3.php?DetectLocation=<?=$_GET["DetectLocation"];?>">&laquo;</a>
</li>

      <li class="page-item">
      <a rel="prev" class="page-link" href="cases3.php?DetectLocation=<?=$_GET["DetectLocation"];?>&page=<?= $prev;?>">&lsaquo;</a>
</li>
...
for($i=$spage; $i<=$page-1; $i++ ): ?>
    ...
    <a rel="next" class="page-link" href="cases3.php?DetectLocation=<?=$_GET["DetectLocation"];?>&page=<?= $i; ?>"><?= $i; ?></a>
  </li>
<?php endfor; ?>
<?php } ?>
...
<?php } ?>
      ...
      <li class="page-item">
      <a rel="next" class="page-link" href="cases3.php?DetectLocation=<?=$_GET["DetectLocation"];?>&page=<?= $next; ?>">&rsaquo;</a>
</li>

      <li class="page-item">
      <a class="page-link" href="cases3.php?DetectLocation=<?=$_GET["DetectLocation"];?>&page=<?= $totoalPages;?>">&raquo;</a>
</li>
<?php  } ?>
  ...
  function response_search_value_input(d, e) {
    ...
    switch (field) {
      case 'expose':
        ...
        $result = $pdo->query("SELECT `cam_ip` FROM `cases` where `DetectLocation`='".$_GET["DetectLocation"]."' GROUP BY `cam_ip`");
while($row = $result->fetch(PDO::FETCH_ASSOC)){
        $resultn = $pdo->query("SELECT `cam_ip`,`Location` FROM `cases` where `cam_ip`='".$row["cam_ip"]."' limit 1");
        $rown = $resultn->fetch(PDO::FETCH_ASSOC);
        echo '<option value="'.$row["cam_ip"].'">'.$row["cam_ip"].$rown["Location"].'</option>';
}
?></select>'
  search_condition.val('eq').change();
  search_condition.find('option').not('[value="eq"]').attr('hidden', '')
  ...
  function review_case(id) {
    ...
    $.post("/wos/admin/cases/batch_expose", data)
  }
  function transform_cases(event) {
    if ($('.select_checkbox:checked').length > 0) {
      if (event == 'review') {
        $.post('/wos/admin/cases/batch_expose', $('.index-ftransform_casesorm').serialize())
        ...
        if ($('[name="cases[department_id]"]').val() == '') {
          ...
          $.post("/wos/admin/cases/assign", { case_ids: ids, department_id: department_id })
        }
      } else if( event == 'complete' ) {
        $('input[name="event"]').val(event);
	$('.index-form').attr('action', "cases3.php?act=complete&page=<?=$_GET["page"];?>").submit();
      } else if( event == 'ready' ) {
        $('input[name="event"]').val(event);
        $('.index-form').attr('action', "cases3.php?act=ready&page=<?=$_GET["page"];?>").submit();
      } else {
        $('input[name="event"]').val(event);
        $('.index-form').attr('action', "cases3.php?page=<?=$_GET["page"];?>").submit();
      }
  ...
  <script type="text/javascript">
    $(function(){
      ...
      function csv_btn() {
        ...
        var url =
          "./php/GetExportCSVCar3.php?" +
          "txt_date1=" +
          $("#txt_date1").val() +
          "&txt_date2=" +
	  $("#txt_date2").val() + "&txt_addr=<?=$_GET["DetectLocation"];?>";
