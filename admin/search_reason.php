...
if( $_POST["_method"] == "delete" ) {
	//$pdo->prepare("delete from `search_reason` where `id`=?")->execute(array($_GET["id"]));
...
<?php
$result = $pdo->query("select * from `search_reason`");
...
<form class="index-form" action="reason_list.php" accept-charset="UTF-8" method="post"><input name="utf8" type="hidden" value="&#x2713;" /><input type="hidden" name="authenticity_token" value="K3Vdn+pD1LWok+ZP4ZVAz0c+BCAiAdLMr7U+yVQD2CFL+YuUshqCTUBY7XKKx+l2e37/KSooDVBN3D02Qt5ojQ==" />
  <div class="card mb-3">
    <div class="card-header">
      ...
      <a class="btn btn-success btn-sm float-right" style="margin-left: 20px;" href="search_reason_add.php">新增事由</a>
...
<?php
$i=0;
$result = $pdo->query("select * from `search_reason` order by `id` ");
...
<?=$i;?>
                    ...
                    <td width="30%" id="username_<?=$row["id"];?>">
<?=$row["name"];?>                      
                    </td>
...
?>
		  ...
		  <a href="search_reason_edit.php?id=<?=$row["id"];?>"><i class="far fa-edit"></i>編輯</a>&nbsp;<!--|&nbsp;<a data-confirm="確定刪除?" rel="nofollow" data-method="delete" href="search_reason.php?id=<?=$row["id"];?>"><i class="fas fa-trash-alt"></i>刪除</a>-->
