...
if( $_GET["min"] != "" && $_GET["max"] != "" && $_GET["full_car_number"] != "" ) {
	        $result = $pdo->query("select * from `cases` where `full_car_number`='".$_GET["full_car_number"]."' and `violated_at`>='".$_GET["max"]."' and `violated_at`<='".$_GET["min"]."' order by `created_at` desc LIMIT $paginationStart, $limit");
} else {
...
<div class="hover_content" style="display: none;">
  ...
  break;
	...
	$result = $pdo->query("SELECT `cam_place` FROM `cases` GROUP BY `cam_place`");
while($row = $result->fetch(PDO::FETCH_ASSOC)){
	$resultn = $pdo->query("SELECT `cam_place`,`Location` FROM `cases` where `cam_place`='".$row["cam_place"]."' limit 1");
	$rown = $resultn->fetch(PDO::FETCH_ASSOC);
	echo '<option value="'.$row["cam_place"].'">'.$rown["Location"].'</option>';
}
?></select>'
  search_condition.val('eq').change();
  search_condition.find('option').not('[value="eq"]').attr('hidden', '')
