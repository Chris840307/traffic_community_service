...
$pdo->prepare("INSERT INTO `sys_log` (`id`, `acc`, `name`, `class`, `ip`, `content`) VALUES (NULL, ?, ?, ?, ?, ?)")->execute(array($row_user["acc"],$row_user["name"],'違規地點',$ip,'修改成功'));
...
$pdo->prepare("UPDATE `areas` SET `name`=?,`code`=?,`ancestry`=?,`active`=?,`updated_at`=? where `id`=?")->execute(array($_POST["report_user"]["name"],$_POST["report_user"]["code"],$_POST["report_user"]["nid"],$active,$cdate,$_POST["report_user"]["id"]));
    ...
    <div id="wrapper">
      ...
      <div id="content-wrapper">
        <div class="container-fluid">
          ...
          <form class="simple_form new_report_user" id="new_report_user" autocomplete="off" action="areas_edit.php" accept-charset="UTF-8" method="post"><input name="utf8" type="hidden" value="&#x2713;" /><input type="hidden" name="authenticity_token" value="XPtXVm/tQsoeHMIWVrQ4Mse296z1CaQD+mz8Y5hlweHkh5ZUmRGaYjiDiCofzvjEtXRYQOmid9nnyq9RMNp+jQ==" />
<div class="card mb-3">
      ...
      <div class="row">
        <div class="col-sm-8">
          ...
          <div class="bgc-white p-20 bd">
            <div class="mT-30">
<?php
$result = $pdo->query("select * from `areas` where `id`='".$_GET["id"]."'");
$row = $result->fetch(PDO::FETCH_ASSOC);
echo '<input type="hidden" name="report_user[id]" value="'.$row["id"].'">';
?>
    <div class="form-group row">
  <label class="col-sm-2 col-form-label col-form-label-md">違規區域</label>
  <div class="col-sm-5">
<select name="report_user[nid]" size="1">
<option value="731" <?php if( $row["ancestry"] == "731" ) echo 'selected';?>>北區</option>
<option value="490" <?php if( $row["ancestry"] == "490" ) echo 'selected';?>>東區</option>
<option value="872" <?php if( $row["ancestry"] == "872" ) echo 'selected';?>>香山區</option>
</select>
  </div>
</div>
    <div class="form-group row">
  ...
  <div class="col-sm-5">
  <input class="form-control <?=$username_msg;?> string required form-control form-control-sm" autocomplete="off" required="required" aria-required="true" type="text" name="report_user[name]" id="report_user_name" value="<?=$row["name"];?>"/>
  </div>
</div>
  ...
  <div class="col-sm-5">
  <input class="form-control <?=$username_msg;?> string required form-control form-control-sm" autocomplete="off" required="required" aria-required="true" type="text" name="report_user[code]" id="report_user_code" value="<?=$row["code"];?>"/>
  </div>
</div>
